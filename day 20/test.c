...
//	printf("%d\n", sizeof(&a + 1));//  4/8  &a是数组的地址，&a+1虽然地址跳过这个数组，但还是地址，所以是4/8个字节
...
//	printf("%d\n", strlen(arr));//随机值
//	printf("%d\n", strlen(arr + 0));//随机值
//	//printf("%d\n", strlen(*arr));//err  数组名解引用就是首元素  a的ASCII是97，然而strlen要的是地址，非法访问内存
//	//printf("%d\n", strlen(arr[1]));//err   传进去98
//	printf("%d\n", strlen(&arr));//随机值    同之前的随机值 
//	printf("%d\n", strlen(&arr + 1));//随机值，相较于之前的随机值减6
//	printf("%d\n", strlen(&arr[0] + 1));//随机值，相较于之前的随机值减1
//
//	return 0;
//}
